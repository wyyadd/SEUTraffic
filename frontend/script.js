 ...
 */
 ...
 var nodeCarNum = document.getElementById("car-num");// 车数量
 var nodeProgressPercentage = document.getElementById("progress-percentage"); //当前模拟进度
 ...
 let nodeCanvas = document.getElementById("simulator-canvas");//模拟画布
 ...
 function start() {
     ...
     uploadFile(replayData, ReplayFileDom.files[0], function(){
         let after_update = function() {
             ...
             hideCanvas();
             try {
                 simulation = JSON.parse(roadnetData[0]);//simulaton存贮道路数据
             } catch (e) {
 ...
 function drawNode(node, graphics) {
     graphics.beginFill(LANE_COLOR);//填充画笔颜色
